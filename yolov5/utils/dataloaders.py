...
class LoadImages:
    ...
    def __init__(self, polygon_roi, path, img_size=640, stride=32, auto=True):
        ...
        self.roi = [(polygon_roi[0][0], polygon_roi[0][1]),\
    ...
    def __next__(self):
        ...
        background = np.full((img0.shape[0], img0.shape[1], 3) , (114,114,114), np.uint8)
...
class LoadStreams:
    ...
    def __init__(self, polygon_roi, sources='streams.txt', img_size=640, stride=32, auto=True):
        self.mode = 'stream'
        self.img_size = img_size
        self.stride = stride
    ...
    def __next__(self):
        ...
        '''
        #assert img0 is not None, 'Image Not Found ' + path
        backgrounds = [np.full((x.shape[0], x.shape[1], 3) , (114,114,114), np.uint8) for x in img0 ]
        #for x in img0:
        #    print(f"x: {x.shape}")
        #    print(f"x: {x}")
        overlays = [x[self.roi[0][1]:self.roi[1][1], self.roi[0][0]:self.roi[1][0]] for x in img0]
        for index in range(len(overlays)):
            backgrounds[index][self.roi[0][1]:self.roi[1][1], self.roi[0][0]:self.roi[1][0]] = overlays[index]
        # Letterbox
        img = [letterbox(x, self.img_size, stride=self.stride, auto=self.rect and self.auto)[0] for x in backgrounds]
